    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_ESCAPE:
                RUN = False
                break
            elif event.key == pygame.K_RIGHT and (idx[0] + 1, idx[1]) in grid[idx].connections:
                history = [idx] + history
                idx = (idx[0] + 1, idx[1])
            elif event.key == pygame.K_LEFT and (idx[0] - 1, idx[1]) in grid[idx].connections:
                history = [idx] + history
                idx = (idx[0] - 1, idx[1])
            elif event.key == pygame.K_DOWN and (idx[0], idx[1] + 1) in grid[idx].connections:
                history = [idx] + history
                idx = (idx[0], idx[1] + 1)
            elif event.key == pygame.K_UP and (idx[0], idx[1] - 1) in grid[idx].connections:
                history = [idx] + history
                idx = (idx[0], idx[1] - 1)
